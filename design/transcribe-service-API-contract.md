...
| 2. 请求契约       | Client to Server 消息结构、字段定义、业务规则 |
| 3. 响应契约       | Server to Client 成功/错误响应结构            |
| 4. 状态码与错误码 | HTTP 握手码、WebSocket 关闭码、应用错误码映射 |
| 5. 完整示例       | 请求与响应对照示例                            |
| 6. Kafka 落盘契约 | Server to Kafka 的消息 Key/Value 与写入规则   |
...
> 完整 Header 列表待定，以下为预留占位；当前实现未启用鉴权校验。


| Header          | 必填       | 类型     | 最大长度 | 说明                                                   |
| --------------- | ---------- | ------ | ---- | ------------------------------------------------------ |
| `Authorization` | 否（预留字段） | string | 4096 | Bearer token 或其他鉴权凭证；待鉴权方案落地后再决定是否转为必填 |



### 1.3 Event Types and Flow
...
8. **结束事件 transcript**：`payload.transcript` 为必填字符串，推荐使用 “EOL”。

---

## 3. Response Body

*Server to Client 消息格式*
...
| E1009  | ERROR     | 403（初始并发发送冲突） / —（握手后策略违规） | 1008（仅握手后） | 是             | 是                     | 不允许的业务操作或策略冲突；<br />- 同会话初始并发发送冲突在握手阶段返回 403;<br />- query 与 `metaData.conversationId` 不一致等握手后违规返回 1008 |
| E1010  | ERROR     | 401                                           | 1008             | 是             | 是                     | 保留错误码；当前实现未启用鉴权校验，待鉴权启用后用于缺少、无效或无权限的凭证 |
| E1011  | ERROR     | 504                                           | 1013             | 是             | 是                     | 上游或下游服务（如 Fano Assist、Kafka）响应超时             |
...
本节描述服务端在成功路径中写入 Kafka 的消息契约，即 **Server to Kafka** 的内部数据格式。
...
| 项目               | 契约                                                         |
| ------------------ | ------------------------------------------------------------ |
| Topic              | 由 `KAFKA_TOPIC` 配置决定，默认 `AI_STAGING_TRANSCRIPTION`   |
...
- 只有通过sequence和conversion owner guard等一系列校验且实际执行 Kafka send 的消息，才会进入 Kafka。
