...
本文档用于防止项目在持续梳理需求、频繁调整实现以及使用 AI 参与改代码的过程中逐步偏离初始设计。

本文档不替代详细设计文档；其作用是固化必须长期稳定的“设计不变量”和“测试护栏”，作为评审、迭代和 AI 参与改码时的固定依据。
...
1. 这次改动触碰了哪些设计不变量？
2. 这次改动新增或修改了哪些测试来证明没有跑偏？
3. 如果行为变化了，哪些文档必须同步更新？

若 AI 无法回答这 3 个问题，就不应直接改业务代码。

### 4.2 优先让 AI 改“测试 + 文档 + 实现”三件套
