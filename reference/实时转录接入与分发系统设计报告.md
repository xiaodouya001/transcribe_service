...
graph TD
    subgraph External[外部环境]
        STT Provider[STT Provider 语音引擎]
    end
    ...
    STT Provider -- "1. 长连接 WebSocket/SSE<br/>持续推送 JSON 转录结果" --> ECS
...
- **ElastiCache (Redis)**：真实网络环境下，STT 引擎重连时经常会把上一秒的话再发一遍。把 Redis 放在这里，就是为了做个快速"滤网"，哪怕它发了两遍，我们也只让一条进 Kafka。

## 2. 核心数据流转时序图 (Sequence Diagram)

这张图展示了从收到 STT Provider 的一句话，到最终落盘到 Kafka 的微秒/毫秒级交互过程。重点看去重防抖的逻辑。

```mermaid
sequenceDiagram
    autonumber
    participant STT Provider as STT Provider
    participant App as ECS Fargate (Python)
    participant Redis as ElastiCache (Redis)
    participant MSK as Amazon MSK (Kafka)

    Note over STT Provider, App: 客户端发起连接，握手成功(WebSocket/SSE)

    loop 持续的音频转录流
        STT Provider->>App: 推送转录 JSON (含 session_id, timestamp, text)
        ...
        alt 发生网络重传，数据已存在 (返回 0)
            ...
            App->>MSK: aiokafka.send() Topic: transcription_topic Key: session_id
            MSK-->>App: ACK (返回 Partition Offset)
        end
    end

    Note over STT Provider, App: 会话结束或主动断开
    App->>Redis: 清理会话残留状态
```
