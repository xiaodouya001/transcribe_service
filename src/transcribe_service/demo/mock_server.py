...
async def inject_handler(request: web.Request) -> web.Response:
    ...
    if data.get("event") == "request" and data.get("data") == "EOF":
        sid = data.get("session_id", "")
        if not sid:
            raise web.HTTPBadRequest(text="EOF payload requires session_id")
        queue = _get_session_queue(request.app, sid)
        await queue.put(data)
        log.info("Mock: 收到 EOF payload", session_id=sid)
...
async def metrics_handler(request: web.Request) -> web.Response:
    """GET /metrics: Demo 监控 - 携程数、Mock 队列数、Connector 活跃会话数。"""
    app = request.app
    coroutines = len(asyncio.all_tasks())
    sessions = len(app["session_queues"])
    connector_sessions = None
    try:
        import httpx
        async with httpx.AsyncClient(timeout=2.0) as client:
            r = await client.get("http://127.0.0.1:8080/metrics")
            if r.status_code == 200:
                data = r.json()
                connector_sessions = data.get("sessions")
    except Exception:
        pass
    return web.json_response({
        "coroutines": coroutines,
        "sessions": sessions,
        "connector_sessions": connector_sessions,
    })
