...
def create_app(connector_manager, *, redis_url: str = "", producer=None) -> FastAPI:
    ...
    async def metrics(request: Request):
        """Demo 监控：返回 ConnectorManager 活跃会话数。"""
        manager = request.app.state.connector_manager
        return {"sessions": len(manager._sessions)}
    ...
    async def webhook_session(request: Request, payload: WebhookPayload) -> JSONResponse:
        ...
        if not session_id:
            log.warning(
                "Webhook: 拒绝请求",
                reason="metadata.session_id required",
                session_id="",
                status_code=400,
            )
            return JSONResponse(
        ...
        shutdown = getattr(manager, "_shutdown", None)
        if shutdown and getattr(shutdown, "draining", False):
            log.info("Webhook: draining 中拒绝", session_id=session_id)
            return JSONResponse(
                status_code=503,
                content={"error": "draining", "session_id": session_id},
            )
        ...
        log.info(
            "Webhook: 已注册会话",
            session_id=session_id,
            sse_url=url if use_sse else None,
            ws_url=url if not use_sse else None,
        )
