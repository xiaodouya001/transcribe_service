...
async def test_claim_refresh_and_release_roundtrip():
    ...
    assert await client.get("transcript:owner:conv-1") == "owner-a"

    await owner.release("conv-1", "owner-a")
    assert await client.get("transcript:owner:conv-1") is None
...
async def test_claim_reacquires_after_ttl_expiry():
    ...
    assert await client.get("transcript:owner:conv-1") == "owner-b"
